...
exports.authenticateFirebaseGoogle = expressAsyncHandler(async (req, res, next) => {
    try {
        const firebaseUser = await admin.auth().verifyIdToken(req.headers.token);
...
exports.getToken = expressAsyncHandler(async (req, res, next) => {
    req.token = jwt.sign({ _id: req.user._id }, config.LOCAL.SECRET, { expiresIn: tokenExpirationTime });
    next();
});
exports.getTokenFunc = (user) => {
 return jwt.sign(user, config.LOCAL.SECRET, { expiresIn: tokenExpirationTime });
}
